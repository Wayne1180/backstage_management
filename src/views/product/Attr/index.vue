...
export default {
  ...
  methods: {
    ...
    addAttrValue() {
      //向属性值的数组里面添加元素
      // attrId:是相应的属性的id，目前我们是添加属性的操作，还没有相应的属性的id，目前带给服务器的id为undefined
      // valuename：相应的属性值的名称
      this.attrInfo.attrValueList.push({
        attrId: this.attrInfo.id, //相应的属性名的ID，对于修改某一个属性的时候，可以在已有的属性值基础之上新增新的属性值（新增属性值的时候，需要把已有的属性的id带上）
        valueName: "",
        //flag属性：给每一个属性值添加一个标记flag，用户切换查看模式与编辑模式。好处：每一个属性值可以控制自己的模式切换
        //当前flag属性，响应式数据（数据变化视图跟着变）
        flag: true,
      });
      this.$nextTick(() => {
    ...
    updateAttr(row) {
      ...
      this.attrInfo.attrValueList.forEach((item) => {
        //这样书写也可以给属性值添加flag字段，但是视图不会跟着变化（因为flag不是响应式数据）
    ...
    toEdit(row, index) {
      row.flag = true;
      //获取input节点，实现自动聚焦
      //点击span的时候，切换为input，变为编辑模式，但是需要注意，对于浏览器而言，页面重绘与重拍耗时间
      this.$nextTick(() => {
        //获取相应的input表单元素实现聚焦
