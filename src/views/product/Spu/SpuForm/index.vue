...
export default {
  ...
  methods: {
    handleRemove(file, fileList) {
      ...
      //收集照片墙图片的数据
      //对于已有的图片，照片墙中显示的图片，有name和url字段的，因为照片墙显示数据务必要有这两个属性
      //对于服务器而言，不需要name。url字段，将来
    ...
    async initSpuData(spu) {
      ...
      if (spuResult.code == 200) {
        //在修改spu的时候，需要向服务器发请求的，把服务器返回的数据（对象），赋值给
  ...
  computed: {
    ...
    unSelectSaleAttr() {
      ...
      //当前SPU拥有的属于自己的销售属性
      //数组的过滤的方法，可以从已有的数组当中过滤出用户需要的元素，并返回一个新的数据
