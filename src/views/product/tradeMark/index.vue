<template>
  <div>
    ...
    >
      <!-- 展示表单元素 -->
      <!-- model 把表单的数据收集到哪个对象身上，将来表单验证也需要这个属性 -->
...
export default {
  ...
  methods: {
    ...
    async getPageList(pager = 1) {
      ...
      //当向服务器发请求时，这个函数需要带参数，因此在data中初始化两个字段，代表给服务器传递参数
      //获取品牌列表的接口
    ...
    updateTradeMark(row) {
      ...
      //将服务器返回品牌的信息直接赋值给了tmForm进行展示
      //也就是说tmform存储即为服务器返回品牌信息
      this.tmForm = { ...row };
    },
    //图片上传成功
